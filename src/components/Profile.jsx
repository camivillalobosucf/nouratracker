...
const GENERO_OPTIONS   = ['Masculino', 'Femenino', 'Prefiero no decir']
...
function cmFromM(str)   { return Math.round(parseFloat(str) * 100) }
function cmFromFtIn(ft, inch) { return Math.round(parseFloat(ft || 0) * 30.48 + parseFloat(inch || 0) * 2.54) }
function mFromCm(cm)    { return (cm / 100).toFixed(2) }
...
function UnitToggle({ unit, onChange }) {
  return (
    <div style={{ display: 'inline-flex', background: '#F7F4EE', border: '1px solid #DDD8CE', borderRadius: 8, overflow: 'hidden', marginBottom: 8 }}>
      {['m', 'ft'].map(u => (
        <button
          key={u}
          onClick={() => onChange(u)}
          style={{
            ...
            fontSize: 12,
            fontWeight: 600,
            background: unit === u ? '#C4714A' : 'transparent',
            color: unit === u ? '#fff' : '#888',
            border: 'none',
            transition: 'all 0.15s',
          }}
        >
          {u === 'm' ? 'Metros' : 'Pies / Pulg.'}
        </button>
      ))}
    </div>
  )
  ...
  return (
    <div className="px-4 pt-10 pb-24">
      ...
      <div style={CARD}>
        ...
        {!pwStep ? (
          <button onClick={() => setPwStep(true)} style={{ fontSize: 14, color: '#C4714A', fontWeight: 500 }}>
            Cambiar contraseña →
          </button>
        ) : (
          <div>
            ...
            />
            {pwMsg && <p className="mb-2 text-sm" style={{ color: pwMsg.ok ? '#7A9E7E' : '#C4714A' }}>{pwMsg.text}</p>}
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => { setPwStep(false); setPwMsg(null); setNewPw('') }}
      ...
      {/* Eliminar cuenta */}
      <div style={{ ...CARD, border: '1px solid #f5c9b8' }}>
        <span style={{ ...LABEL, color: '#C4714A' }}>Zona de peligro</span>
        {!delStep ? (
          <button onClick={() => setDelStep(true)} style={{ fontSize: 14, color: '#C4714A', fontWeight: 500 }}>
            Eliminar cuenta
          </button>
        ) : (
          <div>
            <p className="text-sm mb-3" style={{ color: '#555' }}>
              Se eliminarán permanentemente todos tus datos. ¿Estás seguro?
            </p>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => { setDelStep(false); setDelError('') }}
                style={{ padding: '10px 16px', background: '#F7F4EE', border: '1px solid #DDD8CE', borderRadius: 8, fontSize: 14, color: '#888' }}>
                Cancelar
              </button>
            </div>
...
export default function Profile({ session, isActive }) {
  ...
  const [info, setInfo] = useState({ nombre: '', edad: '', peso_kg: '', genero: '', nivel_actividad: '' })
  const [infoLoading, setInfoLoading] = useState(false)
  const [infoSaved, setInfoSaved]     = useState(false)

  // Height handled separately due to unit conversion
  ...
  const fetchAll = useCallback(async () => {
    const { data } = await supabase.from('user_profile').select('*').eq('user_id', uid).single()
    if (!data) return
    setInfo({
      nombre:          data.nombre          || '',
      edad:            data.edad            || '',
    ...
    if (data.estatura_cm) {
      setEstaturaM(mFromCm(data.estatura_cm))
      const { ft, inch } = ftInFromCm(data.estatura_cm)
      setEstaturaFt(ft)
      setEstaturaIn(inch)
    }
    setMetas(data.metas || [])
  ...
  // When switching units, re-derive the other unit's values from whatever was entered
  const handleUnitSwitch = (u) => {
    if (u === estaturaUnit) return
    if (u === 'ft' && estaturaM) {
      const cm = cmFromM(estaturaM)
      if (!isNaN(cm)) { const { ft, inch } = ftInFromCm(cm); setEstaturaFt(ft); setEstaturaIn(inch) }
    } else if (u === 'm' && estaturaFt) {
      const cm = cmFromFtIn(estaturaFt, estaturaIn)
      if (!isNaN(cm)) setEstaturaM(mFromCm(cm))
    }
    setEstaturaUnit(u)
  }
  ...
  const toggleMeta = async (meta) => {
    const next = metas.includes(meta) ? metas.filter(m => m !== meta) : [...metas, meta]
    setMetas(next)
    setMetasSaving(true)
    await supabase.from('user_profile').upsert(
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input type="text" value={info.nombre}
              ...
              placeholder="Tu nombre" style={INPUT_STYLE} />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <label style={LABEL}>Edad</label>
              <input type="number" value={info.edad}
                onChange={e => setInfo(p => ({ ...p, edad: e.target.value }))}
                placeholder="años" style={INPUT_STYLE} />
            </div>
            <div>
              <label style={LABEL}>Peso actual</label>
              <input type="number" value={info.peso_kg}
                onChange={e => setInfo(p => ({ ...p, peso_kg: e.target.value }))}
                placeholder="kg" style={INPUT_STYLE} />
            </div>
          </div>

          {/* Estatura con toggle de unidades */}
          <div>
            <label style={LABEL}>Estatura</label>
            <UnitToggle unit={estaturaUnit} onChange={handleUnitSwitch} />
            {estaturaUnit === 'm' ? (
              <input type="number" value={estaturaM}
                onChange={e => setEstaturaM(e.target.value)}
                placeholder="ej. 1.75" style={INPUT_STYLE} step="0.01" />
            ) : (
              <div style={{ display: 'flex', gap: 10 }}>
                <div style={{ flex: 1 }}>
                  <input type="number" value={estaturaFt}
                    onChange={e => setEstaturaFt(e.target.value)}
                    placeholder="pies" style={INPUT_STYLE} />
                  <span style={{ fontSize: 11, color: '#aaa', marginTop: 3, display: 'block' }}>pies</span>
                </div>
                <div style={{ flex: 1 }}>
                  <input type="number" value={estaturaIn}
                    onChange={e => setEstaturaIn(e.target.value)}
                    placeholder="pulgadas" style={INPUT_STYLE} />
                  <span style={{ fontSize: 11, color: '#aaa', marginTop: 3, display: 'block' }}>pulgadas</span>
                </div>
              </div>
            )}
          </div>
